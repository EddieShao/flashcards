...
interface StackDao {
    ...
    suspend fun loadStackAndCardsWithStackId(stackId: Int): List<StackAndCards>

    @Query("SELECT * FROM stack WHERE title LIKE '%' || :search || '%'")
    suspend fun loadStacksLikeSearch(search: String): List<Stack>
}