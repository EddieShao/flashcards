...
class StackPagingSource(var query: String, var order: String) : PagingSource<Int, Stack>() {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Stack> {
        ...
        return try {
            val entities = Database.instance.stackDao().pageStacks(query, order, params.loadSize, page * params.loadSize)
            LoadResult.Page(
                data = entities,
